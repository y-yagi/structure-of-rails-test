...
前章では、Railsが提供しているテスト用のクラスについて説明しました。Railsではそれらのクラス以外にもテスト実行する為の機能が提供されています。本章では、これらテストの為の仕組みについて説明します。

== Fixtures

Fixturesはテスト用のデータを作る為の機能です。基本的にはテストで使用する事を想定しています。

テスト用のデータの作成といえばfactory_botが有名です。factory_botが独自DSLでデータを定義するのに対して、FixturesではYAMLでデータの定義を行います。
...
まず"test/fixtures"配下にある、拡張子が"yml"になっているファイルがまとめて読み込まれます。これはサブディレクトリ配下のファイルも全て読み込まれます。

次に、YAMLファイルから対象のテーブル名を取得&データの組立を行います。テーブル名はデフォルトだとファイル名から取得されます。例えば"users.yml"というファイルがあった場合、"users"というテーブルに対してSQLを実行します。

サブディレクトリ配下については、そのサブディレクトリ名が指定された状態でテーブル名が組み立てられます("admin/users.yml"というファイルがあった場合、テーブル名は"admin_users"になる)。

最後に登録されているデータの削除&データの登録を行います。データの削除を行う事で、仮に既にデータが登録されていても、必ずFixtureに定義されたデータのみが登録されている状態にします。この削除&登録は全てのテーブルに対する処理が1つのSQLでまとめて実行されるようになっています。削除&登録も1つのSQLにまとめられています。
...
Railsでは、この問題に対応する為に、異なるスレッドが同じデータベースのコネクションを使用する機能を実装し、Fixturesを使用する場合のみその機能を有効化するようにしています。そのため、Fixturesを使用している場合Capybaraを使用したテストでDBに登録したはずのデータが参照できない、という問題は発生しないようになっています。
...
名前が似ているのですが、これはここまで説明してきたFixturesとは別の、単純にファイルを参照する為の仕組みです。"test/fixtures/files"はYAMLではなくただのファイルを格納する想定のディレクトリである、という事だけご認識頂ければと思います。
...
なお、ディレクトリを指定するとそのディレクトリ配下にあるテストを全て実行するようになっているのですが、"test"ディレクトリを指定してもシステムテストは除外されるようになっています。これはシステムテストは処理が重くなる為、通常のテストスイートとは別に実行するべき、という考えによるものです。システムテストを実行したい場合、"test/system"を明示的に指定してテスを実行するか、@<code>{test:system}タスクを使用してテストを実行する必要があります。

=== minitest plugin

minitestは本体の機能が大分少ないのですが、代わりに拡張機能(plugin)を追加する為の機能がデフォルトで提供されています。

Test Runnerは、幾度かの作り直しの結果、現在はminitestの拡張機能として実装されています。Test Runnerのコードを見たい方はminitestの拡張機能の使い方を把握しておくと読みやすいと思います。minitestの拡張機能については、@<href>{http://y-yagi.tumblr.com/post/87286444980/minitest%E3%81%AEextensions%E3%81%AE%E4%BD%9C%E3%82%8A%E6%96%B9}をご参照下さい。
...
テストを実行するワーカーは@<code>{workers}引数で指定可能です。@<code>{workers}には基本的には数字を指定するのですが、@<code>{:number_of_processors}という値を指定した場合、自動でマシンのコア数の値を指定してくれます。
...
プロセスの場合、プロセスをfork後データベースを新規に作成、及び、schemaのロードを行い、テストではその新規に作成したデータベースを使用するようになります。そのため、プロセス毎に違うデータベースを使用するようになり、プロセス間でデータベースへの処理がが競合しないようになっています。プロセス間でのデータのやりとりには、dRuby@<fn>{druby}が使われるようになっています。なお、作成されたデータベースはテストの後処理で削除されるようになっています。
...
処理の流れは次のようになっています。
 ...
 3. テストの前処理を実施
 4. テストを実行
 5. テスト結果をdRubyのサーバに送信
 6. テストの後処理を実施

3〜6が子プロセスで行われる処理です。3でデーターベースの作成、6でその作成したデーターベースの削除を行います。前処理・後処理はユーザが任意の処理を登録する事も可能です。
...
なお、minitestには元々parallel executorという、スレッドを使って並列でテストをする仕組みがあり、Parallel Testingはその仕組みに依存しています。スレッドの場合の処理はminitestのparallel executorをそのまま使用しているだけです。そのため、他のテストフレームワークでそのまま使用出来るような機能ではありませんのでご注意下さい。
